...
(function ($) {
	...
	$(document).ready(function () {
			...
			if (startDate && endDate) {
				$.ajax({
					...
					beforeSend: function (xhr) {
						$('#btn-filter').text('Sending...');
					},
					success: function (response) {
						$('#btn-filter').text('Send');
						if (response.success) {
							$('#result').html(response.data.posts);
		...
		$('#load-more').click(function (e) {
			e.preventDefault();
			var button = $(this);
			var startDate = $('#start_date').val() || '2000:01:01';
			var endDate = $('#end_date').val() || '2100:01:01';
			var paged = button.data('page') + 1;
				...
				success: function (response) {
					...
					if (response.success) {
						if (response.data.posts) {
							$('#result').append(response.data.posts);
							button.data('page', paged);
							if (!response.data.has_more) {
								button.hide();
							}
						} else {
		...
		// Add datepicker to fields input with id "start_date" и "end_date"
		// $(function () {
